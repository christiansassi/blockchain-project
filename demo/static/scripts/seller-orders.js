        ...
        for(let i=0; i<orderNum; i++) {
            ...
            card.innerHTML = `
                ...
                <div class="order-section bottom">
                    <button class="refund-btn" ${active ? '' : 'disabled'} buyer="${buyer}" id="${id}" price=${order.price} status="${status}">${label}</button>
                </div>
            `;
    ...
    refundButtons.forEach(button => {
        button.addEventListener("click", async function() {
            ...
            try {
                ...
                const buyer = this.getAttribute("buyer");
                const id = this.getAttribute("id");
                const price = this.getAttribute("price");
                const status = this.getAttribute("status");

                if(status == "1")
                    await window.contract.methods.sell(buyer, id, price).send({from: sender});
                else if(status == "2" || status == "6")
                    await window.contract.methods.withdrawOrder(buyer, id).send({from: sender});
